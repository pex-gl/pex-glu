...
function RenderTarget(width, height, options) {
  ...
  if (options.depth) {
    if (options.depth == true) {
      ...
      this.depthAttachments.push(depthBuffer);
    }
    else { //use supplied depth texture
      this.depthAttachments.push(options.depth);
      gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT, gl.TEXTURE_2D, options.depth.handle, 0);
    }
  }
