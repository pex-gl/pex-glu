...
TextureCube.load = function (files, options) {
  var defaultOptions = {
    mipmap: false,
    nearest: false
  };
  ...
  gl.texParameteri(texture.target, TEXTURE_MAX_LEVEL, files.length/6-1);

  for (var i = 0; i < files.length; i++) {
    IO.loadImageData(gl, texture.handle, cubeMapTargets[i%6], files[i], { flip: false, lod: Math.floor(i/6) }, function (image) {
